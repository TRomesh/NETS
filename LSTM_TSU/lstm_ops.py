...
def lstm_cell(n_hidden, number_of_layers, keep_prob):
    cell = tf.contrib.rnn.BasicLSTMCell(n_hidden, forget_bias=1.0, activation=tf.tanh, state_is_tuple=True)
    cell = tf.contrib.rnn.DropoutWrapper(cell, output_keep_prob=keep_prob)
    return tf.contrib.rnn.MultiRNNCell([cell] * number_of_layers, state_is_tuple=True)
...
def x_to_s(x_input, params):
    ...
    x_tr_reshape_split = tf.split(axis=0, num_or_size_splits=max_time_step, value=x_tr_reshape)
...
def x_to_wsu(x_input, y_title, user_idx, params):
    ...
    print('Input tensor', x_reshape, 'dimension', str(dim_we + dim_se + dim_ue))

    # linear transformation
    x_linear = linear(x_reshape, output_dim=dim_hidden, activation=tf.nn.sigmoid, scope='input-linear')
    x_split = tf.split(axis=0, num_or_size_splits=max_time_step, value=x_linear)
...
def post_concat_lstm(x_input, y_title, x_length, user_idx, lstm_dropout, params, scope):
    ...
    with tf.variable_scope(scope or 'lstm'):
        ...
        outputs, state = tf.contrib.rnn.static_rnn(cell, x_input, sequence_length=x_length, dtype=tf.float32)
        ...
        concat = tf.concat(axis=2, values=[outputs, y_title, user_embed])
...
def post_concat_mlp(x_input, y_title, user_idx, params, scope):
    ...
    with tf.variable_scope(scope or 'lstm'):
        outputs = tf.transpose(tf.stack(x_input), [1, 0, 2])
